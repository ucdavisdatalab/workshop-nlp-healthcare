...
download data here:\
[abstracts-datafied.csv](/abstracts-datafied.csv)
...
```{r}
#install.packages(c('tm', 'readr', 'pcaMethods'))
library('tm')
library('readr')
library('pcaMethods')
```
...
```{r}
data <- read_csv("data/abstracts-datafied.csv")
...
```{r}
mindx <- order(plotdata$PC1, decreasing=FALSE)[1:3]
maxdx <- order(plotdata$PC1, decreasing=TRUE)[1:3]
